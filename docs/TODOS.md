...
**期望行为**：
1. **解除运行中切换限制**：
   - 允许用户在任一会话正在执行时，自由在侧栏切换到其他会话进行查阅、复盘或在空闲会话中开启新任务；
2. **按会话隔离 ViewState 与事件路由**：
   - 将 `items`、`currentTurn`、`activeTodos`、`sessionUsage`、`running` 改为按 `sessionId` 隔离的字典/池化管理；
   - 后台运行的会话继续在内存中累积事件并推进状态，用户切回该会话时无缝呈现最新进度；
3. **侧栏多会话状态实时指示**：
   - 侧栏每个会话行独立呈现状态：如「● 运行中（绿点脉动）」、「○ 等待审批（橙色呼吸）」、「✕ 出错」等，点击直达对应任务现场；
4. **工作区写入互斥与并发安全（Workspace Lock）**：
   - 绑定不同工作区（项目文件夹）的会话允许完全并发运行；
   - 绑定同一工作区根目录的多个会话，在执行写文件（`write_file`/`edit_file`）或终端命令（`run_command`）时引入工作区锁与排队等待机制，防止多个 Agent 同时修改同一文件产生竞态写冲突。

**涉及改动**：
- `packages/ui/src/store.ts`：会话状态池化重构与事件路由分流；
- `packages/ui/src/components/Sidebar.tsx`：多会话状态标识与非阻塞切换；
- `packages/engine/src/server.ts`：工作区锁与多会话并发协调。
