...
**期望行为**：
1. **折叠头部 Sticky 吸顶**：
   - 工具卡片头部（`.tool-head`）、思考过程头部（`details.thinking summary`）以及回合头部（`.turn-head`）在对应容器可见期间开启吸顶（`position: sticky; top: 0`）；
   - 当用户顺着滚轮向下阅读长内容时，折叠头平滑贴附在可视视口顶部常驻，不再消失；
2. **防穿模与层级视觉保障**：
   - 吸顶头部设置实色背景（如 `background: var(--panel)` 或 `var(--panel2)`），并提供轻微的下分割线或层次阴影，确保下方滚动的文字与代码平滑滑入头部下方而不穿模透出；
3. **随时一键收拢**：
   - 无论用户向下滚动了多长，只要在当前卡片范围内，折叠条始终触手可及，**看完直接在视口顶部点击即可收起**，无需任何往回滑动的成本。

**涉及改动**：
- `packages/ui/src/styles.css`（为 `.tool-head`、`.thinking summary`、`.turn-head` 添加 `position: sticky; top: 0; z-index: 5` 与不透明背景、阴影规则）
