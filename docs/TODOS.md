...
**期望行为**：
1. **输入触发下拉**：在 Composer 输入框中输入 `@` 时，在光标位置上方即时唤出终端风格的悬浮检索面板；
2. **模糊检索工作区文件**：
   - 基于当前会话绑定的工作区目录（`workspaceRoot`），快速检索文件树；
   - 过滤 `.git`, `node_modules`, `dist` 等常见忽略目录；
   - 支持多段字符模糊匹配（Fuzzy Search），支持文件名与目录路径匹配；
3. **纯键盘操作导航**：
   - 支持键盘 `↑` / `↓` 移动选项目，`Enter` 或 `Tab` 键选中，`Esc` 关闭弹出层；
   - 选中后自动将相对路径（如 `@packages/ui/src/App.tsx `）填充至当前光标处并自动后置空格，方便继续输入；
4. **轻量与响应式**：检索过程防抖，支持未绑定工作区时的友好提示（“当前未绑定工作区”）。

**涉及改动**：
- `packages/ui/src/components/Composer.tsx`：键盘 `@` 字符监听、光标位置测量、检索浮层、键盘劫持；
- 工作区文件检索：利用现有 `search_files` 或 Host 的 glob/遍历能力获取文件树。
...
**期望行为**：
1. **分叉操作入口**：
   - 在每个历史回合头部（`TurnView`）或用户消息操作条上，提供「⑂ 从此处分叉（Fork）」按钮；
2. **无损克隆上下文**：
   - 点击后，引擎提取从会话起始到该指定消息为止的全部消息历史（包括前面的上下文、工具结果、已读文件状态）；
   - 创建为一个全新的会话实例，标题默认继承原标题并增加 `[分支]` 或 `(Fork)` 后缀；
   - 继承原会话的工作区绑定与模型/思考档位配置；
3. **即时无缝切换**：
   - 创建成功后，自动在侧栏高亮并切换到新分支会话，用户即可在此处输入新的分支方案指令继续交互；
   - 原会话完整保留，用户可随时在侧栏切换两套方案进行比对。

**涉及改动**：
- `packages/engine/src/server.ts`：新增 `forkSession(sessionId, upToMessageId)` 核心方法；
- `packages/ui/src/store.ts` 与 `Transcript.tsx`：分叉按钮入口与跳转联动。
...
**期望行为**：
1. **Token 估算异步化/缓存化/增量化**：
   - 对已定型的历史消息与工具输出结果计算 Token 后进行哈希缓存，避免反复计算；
   - 流式生成期间仅对当前新增增量做快速估算，禁止在每次 Re-render 时全量跑 BPE 分词；复杂计算考虑移入 Web Worker 或 Rust/Node 宿主端；
2. **流式事件 RAF / 50ms 批量合并（Batching）**：
   - 在 `store.handleEvent` 对 `text_delta`、`reasoning_delta` 等高频流式事件引入批处理缓冲区，利用 `requestAnimationFrame` 或 50ms 节流窗口合并更新，将每秒几十次重绘收敛到流畅的渲染帧率；
3. **消除布局抖动与滚动节流**：
   - 将 `Transcript` 滚动监听与视口刻度高亮改用 `IntersectionObserver` 或严格的 RAF 节流，坚决杜绝在滚轮事件中同步循环调用 `getBoundingClientRect()`；
4. **代码块与 Markdown 渲染分块缓存**：
   - 流式过程中已闭合的代码块使用 `React.memo` / 内容哈希进行高亮结果缓存，仅对当前正在生成的代码块做增量语法渲染；
5. **引入虚拟滚动（Virtual List）评估与 DOM 惰性挂载**：
   - 对长对话列表按回合或视口范围实施 DOM 惰性挂载，彻底卸载视窗外已折叠卡片的庞大 DOM 节点。

**涉及改动**：
- `packages/ui/src/components/ContextChip.tsx`：Token 估算缓存与异步化；
- `packages/ui/src/store.ts`：流式事件批处理合并与节流通知；
- `packages/ui/src/components/Transcript.tsx`：移除密集 getBoundingClientRect，优化滚动监听；
- `packages/ui/src/markdown.ts` 与相关渲染组件：代码高亮缓存。
...
**期望行为**：
1. **多级阈值监测与预警**：
   - 当会话消耗达到上下文容量的 `80%` 时，在界面与 `ContextChip` 呈现视觉预警；
   - 当达到 `90%`（阈值可配置）且 Agent 仍在多步循环中时，自动激活上下文压缩流程；
2. **阶梯式渐进压缩机制**：
   - **第一级（工具长输出折叠裁切）**：优先修剪历史较早轮次中庞大的工具输出（如几千行代码的 `read_file`、大型编译日志），替换为结构化摘要占位符（如 `[输出已归档折叠，共 520 行，包含关键定义: ...]`);
   - **第二级（历史会话智能摘要 Compaction）**：调用轻量/快速模型，将较早的历史轮次聚合成结构化「会话阶段记忆」（已实现改动、关键决策、当前未决问题），替换旧的原始消息，仅保留最近 3~5 轮完整交互和最新的任务清单；
3. **手动压缩瘦身入口**：
   - 在 `ContextChip` 浮层中增加「压缩上下文」操作按钮，允许开发者在进入大任务前主动瘦身并腾出 Token 预算。

**涉及改动**：
- `packages/core/src/loop.ts`：循环步间的超限检测与压缩拦截点；
- `packages/engine/src/server.ts`：上下文压缩器（Compactor）与摘要提示词驱动；
- `packages/ui/src/components/ContextChip.tsx`：手动压缩操作入口与视觉警示。
